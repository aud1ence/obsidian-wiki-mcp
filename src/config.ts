...
function loadConfigFile(): Partial<Config> {
  ...
  try {
    ...
    if (parsed.vault_path) parsed.vaultPath = expandHome(parsed.vault_path);
    return parsed;
  } catch {
    return {};
  }
...
export function resolveConfig(): Config {
  ...
  const vaultFromCli =
    vaultArgIdx !== -1 ? expandHome(args[vaultArgIdx + 1]) : undefined;
  ...
  if (!vaultPath) {
    console.error(
      "[obsidian-wiki-mcp] ERROR: vault_path is required.\n" +
  ...
  return {
    ...
    logLevel:
      (process.env.WIKI_LOG_LEVEL as Config["logLevel"]) ??
      fileConfig.logLevel ??
    ...
    staleLockTtlMs:
      fileConfig.staleLockTtlMs ?? DEFAULT_CONFIG.staleLockTtlMs,
  };
}
