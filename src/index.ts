...
async function main() {
  ...
  if (!fs.existsSync(config.vaultPath)) {
    console.error(
      `[obsidian-wiki-mcp] ERROR: Vault path không tồn tại: ${config.vaultPath}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      `[obsidian-wiki-mcp] ERROR: Không có quyền đọc/ghi vault: ${config.vaultPath}`
    );
    process.exit(1);
  }
  ...
  if (!vaultInit) {
    console.error(
      `[obsidian-wiki-mcp] WARN: Vault chưa được khởi tạo (không có _schema.md). Chạy wiki_init() để khởi tạo.`
    );
  }
