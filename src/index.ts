...
async function main() {
  ...
  if (!fs.existsSync(config.vaultPath)) {
    console.error(
      `[obsidian-wiki-mcp] ERROR: Vault path does not exist: ${config.vaultPath}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      `[obsidian-wiki-mcp] ERROR: No read/write permissions for vault: ${config.vaultPath}`
    );
    process.exit(1);
  }
  ...
  if (!vaultInit) {
    console.error(
      `[obsidian-wiki-mcp] WARN: Vault not initialized (missing _schema.md). Run wiki_init() to initialize.`
    );
  }
