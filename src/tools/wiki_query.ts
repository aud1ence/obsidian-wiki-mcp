...
function fullTextScan(
  vaultPath: string,
  query: string,
  topK: number
): Array<{ path: string; tldr: string; score: number }> {
  const pages = listWikiPages(vaultPath);
  const queryTerms = query.toLowerCase().split(/\s+/);
  const results: Array<{ path: string; tldr: string; score: number }> = [];
...
export function registerWikiQuery(
            ...
            text: JSON.stringify(
              {
                results: enriched,
                next_step:
                  "TL;DRs above may be sufficient. Call wiki_read_page(path, 'full') for detailed content if needed.",
              },
              null,
              2
            ),
