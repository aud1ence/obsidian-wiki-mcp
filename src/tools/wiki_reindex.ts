...
import { Bm25Index, IndexRow, writeIndexFile } from "../lib/index_manager.js";
import { BacklinkIndex, buildBacklinkIndex } from "../lib/backlink_index.js";
import { isVaultInitialized, listWikiPages, relPath } from "../lib/vault.js";
import { appendLog } from "../lib/log_manager.js";

export function registerWikiReindex(
  ...
  server.registerTool(
    ...
    {
      description:
        "Rescan all pages in _wiki/ and rebuild BM25 index + _index.md. " +
      ...
      inputSchema: {
        dry_run: z
          ...
          .optional()
          .describe(
            "If true, report what would be indexed without writing anything. Default: false."
          ),
      },
    },
    async (args) => {
      ...
      for (const absPath of pages) {
        ...
        try {
          ...
          const last_modified =
            typeof fm.last_modified === "string" ? fm.last_modified : today;
