...
- [Hướng dẫn CLAUDE.md](#hướng-dẫn-claudemd)
...
### Cách 2: Chỉnh trực tiếp file JSON (`~/.claude.json`)

Thêm vào `~/.claude.json` trong phần `mcpServers` của project tương ứng:
...
Hoặc áp dụng cho tất cả project (global), thêm vào `mcpServers` ở cấp root:
...
> **Lưu ý:** Nếu vault path có khoảng trắng, giữ nguyên trong một phần tử của mảng `args` — không được tách thành nhiều phần tử.
...
Thêm file `CLAUDE.md` vào vault giúp enforce đúng workflow MCP tool cho mọi session Claude Code — không cần nhắc lại mỗi lần.

### Tại sao cần thiết

Nếu không có `CLAUDE.md`, Claude mặc định dùng các tool file thông thường (`Write`, `Edit`, `Read`) khi làm việc trong vault. Điều này bỏ qua tính năng tự động index, quản lý backlink và kiểm tra schema của MCP — khiến `_index.md` bị lệch và search bị hỏng.

### Nội dung `CLAUDE.md` được khuyến nghị

Tạo file `CLAUDE.md` ở thư mục gốc của vault với nội dung:

```markdown
# CLAUDE.md

## Obsidian Wiki MCP — Bắt buộc

Khi làm việc với wiki page trong vault này, luôn dùng obsidian-wiki MCP tools.
KHÔNG dùng Write, Edit hoặc Read trực tiếp cho các file trong `_wiki/`.

### Workflow chuẩn khi ghi wiki

1. `wiki_query` — tìm page liên quan trước (tránh trùng lặp, tìm backlinks)
2. `wiki_write_page` — ghi page (tự động cập nhật `_index.md` và backlinks)
3. KHÔNG tự tay edit `_index.md` hay `_log.md`

### Mapping tool

| Thay vì              | Dùng               |
|----------------------|--------------------|
| `Write` vào `_wiki/` | `wiki_write_page`  |
| `Read` file wiki     | `wiki_read_page`   |
| Tìm kiếm trong wiki  | `wiki_query`       |
| Import file thô      | `wiki_ingest`      |
```

### Mỗi rule ngăn chặn điều gì

| Rule | Nếu không có |
|------|--------------|
| `wiki_query` trước khi ghi | Page trùng lặp với nội dung chồng chéo |
| `wiki_write_page` thay vì `Write` | `_index.md` và `_log.md` bị lệch |
| Không edit `_index.md` thủ công | BM25 index bị hỏng |

### Sau khi thêm `CLAUDE.md`

Kiểm tra hoạt động: yêu cầu Claude ghi một wiki page và xác nhận Claude gọi `wiki_query` trước, sau đó `wiki_write_page` — không phải tool `Write`.
